...
import { fetchExercises, dropExercises } from "../../redux/actions/exercisesAction"
import { fetchTrainings, dropTrainings } from "../../redux/actions/trainingsAction"
  ...
  return (
    <div>
      <div className="d-lg-none mb-3 mobile">
        ...
        <img onClick={onToggleMenu} src={hamburger} alt="" className="col-2" ></img>
        <div hidden={!menu}>
          <Link className={(window.location.pathname === '/training' ? "nav-link selected-link" : "nav-link")} to="/training">
            <h3>training</h3>
          </Link>
          <Link className={(window.location.pathname === '/history' ? "nav-link selected-link" : "nav-link")} to="/history">
            <h3>history</h3>
          </Link>
          <Link className={(window.location.pathname === '/exercises-list' ? "nav-link selected-link" : "nav-link")} to="/exercises-list">
            <h3>exercises list</h3>
          </Link>
          <Link className={(window.location.pathname === '/new-exercise' ? "nav-link selected-link" : "nav-link")} to="/new-exercise">
            <h3>new exercise</h3>
          </Link>
      ...
      <div className="mt-3 d-none d-lg-inline">
        {localStorage ? '' : <Redirect to="/" />}
        <img src={default_avatar} alt="" className="ml-5 d-none d-md-inline" ></img>
        <Link className={(window.location.pathname === '/training' ? "nav-link selected-link" : "nav-link")} to="/training">
          <h3>training</h3>
        </Link>
        <Link className={(window.location.pathname === '/history' ? "nav-link selected-link" : "nav-link")} to="/history">
  ...
  function logout() {
    ...
    if (localStorage) {
      if (localStorage.userSession) {
        ...
        axios
          .delete(process.env.REACT_APP_API_URL + '/api/users/logout/' + userSession._id)
          .then(res => {
            if (res.data.success) {
