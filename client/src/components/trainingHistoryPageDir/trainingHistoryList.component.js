...
function HistoryList(props) {
    ...
    useEffect(() => {
        if (trainings) {
            setTrainingsList(trainings.filter((training) => training.finished === true));
        }
    }, [trainings])

    const sortedTrainingsList = trainingsList.sort((a, b) => new Date(b.date) - new Date(a.date));
        ...
        <div>
            <div className="d-none d-lg-block col-12 my-3 pb-3 training-container training-list-header ">
                <div className="row">
                    ...
                    <button onClick={toggleChange} className="btn btn-outline-dark p-3 m-2">{toggle ? 'fold' : 'expand'}</button>
                    <button onClick={deleteTraining} className="btn btn-outline-danger p-3 m-2">delete</button>
                </div>
            ...
            <div className="d-lg-none col-12 my-2 pb-3 training-container training-list-header mobile">
                <div className="row">
                    ...
                    <button onClick={toggleChange} className="btn btn-outline-dark col-4 p-1  m-2 btn-mobile">{toggle ? '↑' : '↓'}</button>
                    <button onClick={deleteTraining} className="btn btn-outline-danger col-4 p-1 m-2 btn-mobile">-</button>
                    </div>
                </div>
...
function Exercises(props) {
    ...
    useEffect(() => {
        if (exercises) {
            setExercisesList(exercises.filter((exercise) => props.training.exercises.includes(exercise._id)));
        }
                        ...
                        return (
                            <div key={exercise._id} className="exercise-row">
                                <div className="col-12 px-0 mt-1 pb-2 d-flex justify-content-between exercise-container exercise-list-header">
                                    ...
                                    <label hidden={exercise.description === ''} className="col-5 p-0 vertically-middle">description</label>
                                    <label hidden={exercise.group.posses === false} className="col-3 p-0 vertically-middle">group</label>
                                </div>
                                <div className="col-12 px-0 mt-1 pb-2 d-flex justify-content-between">
                                    ...
                                    <label hidden={exercise.description === ''} className="col-5 p-0 vertically-middle">{exercise.description}</label>
                                    <label hidden={exercise.group.posses === false} className="col-3 p-0 vertically-middle">{exercise.group.name}</label>
                                </div>

                                <div className="col-12 px-0 mt-1 pb-2 d-flex justify-content-around exercise-container exercise-list-header">
                                    <label hidden={exercise.repetition.posses === false} className="p-0 vertically-middle ">repetition</label>
                                    <label hidden={exercise.weight.posses === false} className="p-0 vertically-middle ">weight</label>
                                    <label hidden={exercise.distance.posses === false} className="p-0 vertically-middle ">distance</label>
                                    <label hidden={exercise.duration.posses === false} className="p-0 vertically-middle ">duration</label>
                                </div>
                                <div className="col-12 px-0 mt-1 pb-4 d-flex justify-content-around">
                                    <label hidden={exercise.repetition.posses === false} className="p-0 vertically-middle ">{exercise.repetition.amount}</label>
                                    <label hidden={exercise.weight.posses === false} className="p-0 vertically-middle ">{exercise.weight.amount}</label>
                                    <label hidden={exercise.distance.posses === false} className="p-0 vertically-middle ">{exercise.distance.amount}</label>
                                    <label hidden={exercise.duration.posses === false} className="p-0 vertically-middle ">{exercise.duration.amount}</label>
                                </div>
                            </div>
                        );
                    })
